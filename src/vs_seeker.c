...
bool8 UpdateVsSeekerStepCounter(void)
{
    ...
    {
        if ((gSaveBlock1Ptr->trainerRematchStepCounter & 0xFF) < 1)
            gSaveBlock1Ptr->trainerRematchStepCounter++;
    }

    if (FlagGet(FLAG_SYS_VS_SEEKER_CHARGING) == TRUE)
    {
        if (((gSaveBlock1Ptr->trainerRematchStepCounter >> 8) & 0xFF) < 1)
        {
...
{
    gSaveBlock1Ptr->trainerRematchStepCounter &= 0x00FF;
    gSaveBlock1Ptr->trainerRematchStepCounter |= (1 << 8);
}
...
static u8 CanUseVsSeeker(void)
{
    ...
    {
        TV_PrintIntToStringVar(0, 1 - vsSeekerChargeSteps);
